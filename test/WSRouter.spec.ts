...
describe('WSRouter', () => {
  ...
  it('addLiquidity', async () => {
    ...
    expect(await pairToken.balanceOf(wallet.address)).to.eq(expectedLiquidity.sub(MINIMUM_LIQUIDITY))
  })
  ...
  it('addLiquidityETH', async () => {
    ...
    expect(await WETHPairToken.balanceOf(wallet.address)).to.eq(expectedLiquidity.sub(MINIMUM_LIQUIDITY))
  })
  ...
  it('removeLiquidity', async () => {
    ...
    const expectedLiquidity = expandTo18Decimals(2)
    await pairToken.approve(router.address, MaxUint256)
    await expect(
    ...
    expect(await pairToken.balanceOf(wallet.address)).to.eq(0)
  ...
  it('removeLiquidityETH', async () => {
    ...
    const expectedLiquidity = expandTo18Decimals(2)
    const WETHPairToken0 = await WETHPair.token0()
    await WETHPairToken.approve(router.address, MaxUint256)
    await expect(
    ...
    expect(await WETHPairToken.balanceOf(wallet.address)).to.eq(0)
  ...
  it('removeLiquidityWithPermit', async () => {
    ...
    const nonce = await pairToken.nonces(wallet.address)
  ...
  it('removeLiquidityETHWithPermit', async () => {
    ...
    const nonce = await WETHPairToken.nonces(wallet.address)
    const digest = await getApprovalDigest(
      WETHPairToken,
      { owner: wallet.address, spender: router.address, value: expectedLiquidity.sub(MINIMUM_LIQUIDITY) },
      nonce,
  ...
  describe('swapExactTokensForTokens', () => {
    ...
    it('gas', async () => {
      ...
      expect(receipt.gasUsed).to.eq(109711)
    }).retries(3)
  })
  ...
  describe('swapExactETHForTokens', () => {
    ...
    it('gas', async () => {
      ...
      const receipt = await tx.wait()
      expect(receipt.gasUsed).to.eq(149029)
    }).retries(3)
  })
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100))).to.be.revertedWith(
      'WSLibrary: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100))).to.be.revertedWith(
      'WSLibrary: INSUFFICIENT_OUTPUT_AMOUNT'
    )
  ...
  it('getAmountsIn', async () => {
    ...
    await expect(router.getAmountsIn(BigNumber.from(1), [token0.address])).to.be.revertedWith('WSLibrary: INVALID_PATH')
    const path = [token0.address, token1.address]
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    pair = new Contract(pairAddress, JSON.stringify(IWSPair.abi), provider).connect(wallet)
    pairToken = new Contract(pairAddress, JSON.stringify(IWSERC20.abi), provider).connect(wallet)
  })
  ...
  it('removeLiquidityETHSupportingFeeOnTransferTokens', async () => {
    ...
    const liquidity = await pairToken.balanceOf(wallet.address)
